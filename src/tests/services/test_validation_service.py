...
class TestValidationService(unittest.TestCase):
    ...
    def test_empty_entries_valid(self):
        ref = MockReference("Key1", 2020, "John Smith", "A Valid Title", RefType.ARTICLE)
    ...
    def test_year_range_valid(self):
        ref = MockReference("Key1", 2020, "John Smith", "A Valid Title", "article")
        self.assertTrue(ValidationService._validate_year_range(datetime.now().year, ref))

    def test_year_range_too_low(self):
        ref = MockReference("Key1", 999, "John Smith", "A Valid Title", "article")
        self.assertFalse(ValidationService._validate_year_range(datetime.now().year, ref))
    ...
    def test_citation_key_starts_with_letter_valid(self):
        self.assertTrue(ValidationService._validate_citation_key_starts_with_letter("A123"))

    def test_citation_key_starts_with_letter_invalid(self):
        self.assertFalse(ValidationService._validate_citation_key_starts_with_letter("1ABC"))

    def test_citation_key_unique_valid(self):
        self.assertTrue(ValidationService._validate_citation_key_unique("Key3", ["Key1", "Key2"]))

    def test_citation_key_unique_invalid(self):
        self.assertFalse(ValidationService._validate_citation_key_unique("Key1", ["Key1", "Key2"]))
    ...
    def test_validate_reference_invalid_year(self):
        ref = MockReference(
            "Key1", 999, "John Smith", "A Valid Title", RefType.ARTICLE
        )
        with self.assertRaises(UserInputError):
            ValidationService.validate_reference(ref, [])
